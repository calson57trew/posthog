...
const sessionRecordingSummarize = (): ToolBase<typeof SessionRecordingSummarizeSchema, Schemas.SessionSummaries> => withUiApp('session-summary', {
    ...
    handler: async (context: Context, params: z.infer<typeof SessionRecordingSummarizeSchema>) => {
        const projectId = await context.stateManager.getProjectId()
        const body: Record<string, unknown> = {}
        if (params.session_ids !== undefined) {body["session_ids"] = params.session_ids}
        if (params.focus_area !== undefined) {body["focus_area"] = params.focus_area}
