...
Mirrors the pattern in `posthog/temporal/session_replay/session_summary/state.py` —
copied rather than imported so Vision owns its own key namespace, TTL, and labels.
"""
...
class StateActivitiesEnum(Enum):
    SESSION_EVENTS = "session_events"  # Analytics events for the session, fetched from ClickHouse.
...
def get_redis_state_client(
    label: StateActivitiesEnum | None = None,
    state_id: str | None = None,
) -> tuple[aioredis.Redis, str | None]:
    """Return the Vision Redis client and an optional generated state key."""
    redis_client = get_async_client(settings.REPLAY_VISION_REDIS_URL)
    key = generate_state_key(label=label, state_id=state_id) if label and state_id else None
